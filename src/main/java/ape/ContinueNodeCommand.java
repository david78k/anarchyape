...
public class ContinueNodeCommand extends ApeCommand
{
	...
	public ContinueNodeCommand()
	{
		...
		.withValueSeparator()
        .withDescription("Continues a tasktracker or a datanode at the given hostname that has already been suspended")
        .withLongOpt("continue-node")
        .create("e");
	}
	...
	public boolean runImpl(String [] args) throws IOException  
	{
				...
				nodeType.toLowerCase().equals("namenode"))
		{
			...
			String cmd = "echo \"kill -18 \\$(ps aux | grep -i " + nodeType + " | grep -v grep | grep -v ape | awk -F ' '  '{print \\$2}')\" > kill-node.sh && chmod +x kill-node.sh && ./kill-node.sh";
